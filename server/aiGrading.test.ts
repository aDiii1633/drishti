...
const question = {
  ...
  questionText:
    "Explain the working-capital cycle and its effect on a business.",
  maximumMarks: 5,
  ...
  criteria: [
    {
      ...
      score: 2,
      maximumScore: 2,
      evidence: "Cash is tied up until sales are collected.",
    },
    {
      criterionId: "Q4-b",
      status: "partial",
      score: 1,
      maximumScore: 2,
      evidence: "The answer mentions inventory but not receivables.",
    },
    {
      criterionId: "Q4-c",
      status: "partial",
      score: 0.5,
      maximumScore: 1,
      evidence: "It says slower conversion reduces available cash.",
    },
  ],
...
function configureGemini() {
  // These cases cover the Gemini image path, which stays available for
  // vision-based evidence mapping; Suprsonic is the default provider.
...
describe("Gemini question-first grading", () => {
  ...
  it("requires review for unreadable or low-confidence evidence without inventing marks", async () => {
    ...
    expect(result.grade.warnings.join(" ")).toMatch(
      /confidence|mapping|readability/i
    );
  });
  ...
  it("uses the stored SVG transcript for local demonstration answer sheets", () => {
    ...
    expect(answerPageTextFallback(page)).toContain(
      "Inventory becomes cash after collection."
    );
  });
